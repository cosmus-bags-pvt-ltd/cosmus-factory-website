...
class ItemBinAssignmentForm(forms.ModelForm):
    bins = forms.ModelMultipleChoiceField(
        queryset=bin_for_raw_material.objects.all(),
        widget=forms.CheckboxSelectMultiple(attrs={"class": "bin-checkbox"}),
        required=False,
        label="Select Bins"
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if item:
            assigned_bins = ItemBinAssignment.objects.filter(item=item).values_list("bin", flat=True)
            self.fields["bins"].initial = assigned_bins

        # Set human-readable bin names
        self.fields["bins"].queryset = bin_for_raw_material.objects.all()
        self.fields["bins"].label_from_instance = lambda obj: f"{obj.bin_name}"

    class Meta:
        model = ItemBinAssignment
        fields = ["bins"]
